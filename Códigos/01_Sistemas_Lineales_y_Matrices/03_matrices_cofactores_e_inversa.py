...
[Texto Guía — Stanley Grossman]     : Teorema fundamental de la adjunta A*Adj(A) = det(A)*I,
...
class MatrixCofactorInverter:
    ...
    def laplace_expansion_steps(self, row_idx: int = 0) -> List[str]:
        ...
        steps.append("Fórmula: det(A) = " + " + ".join(terms_formula))
        steps.append("Sustitución: det(A) = " + " + ".join(terms_num) + f" = {total}")
        return steps

    def plot_matrix_diagram(self, save_path: Optional[str] = None, show_plot: bool = False):
        """
        ...
        fig.suptitle(
            ...
            f"Orden {self.n}x{self.n} | det(A) = {self.det_A} "
        ...
        for idx, ax in enumerate(axes.flat):
            ...
            ax.imshow(mat_num, cmap=cmaps[idx], vmin=-max_abs, vmax=max_abs, aspect='auto')
...
def run_demonstration():
    ...
    print("--- Módulo 03: Matrices, Cofactores e Inversión Matricial ---")
    matrix_example = [
        [1, 2, -1],
        [2, 0, 1],
        [1, 1, 1]
    ...
    print("\nDesarrollo de Laplace por Fila 1:")
    for step in inverter.laplace_expansion_steps(row_idx=0):
        print(f"  {step}")

    print(f"\nDeterminante: det(A) = {inverter.det_A} (Invertible / No singular)\n")

    inv, report = inverter.compute_inverse()

    print("Matriz de menores complementarios M = (det(M_ij)):")
    sp.pprint(report["minors"])

    print("\nTablero de signos S = (-1)^(i+j):")
    sp.pprint(report["signs"])

    print("\nMatriz de cofactores Cof(A) = (C_ij):")
    sp.pprint(report["cofactors"])

    print("\nMatriz adjunta Adj(A) = [Cof(A)]^T:")
    sp.pprint(report["adjugate"])

    print(f"\nMatriz inversa A^(-1) = (1/{inverter.det_A}) * Adj(A):")
    sp.pprint(inv)
